 ...
 */
internals.checkForExistingValues = async function ({ username, email, id }, txModel) {
    const result =
        username || email
            ? await txModel
                  .query()
...
exports.authenticate = async function ({ username, password }) {
    ...
    return Authentication.getToken(user.id, user.username);
};
...
exports.create = function (user) {
    return transaction(UserDao.getModel(), async txUser => {
        await internals.checkForExistingValues(user, txUser);
        const { password } = user;
...
exports.edit = function (id, updatedUser) {
    return transaction(UserDao.getModel(), async txUser => {
        ...
        await internals.checkForExistingValues({ id, ...updatedUser }, txUser);
