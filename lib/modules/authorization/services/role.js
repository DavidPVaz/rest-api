...
exports.remove = function (id) {
    return Repository.tx(Repository.User.model, async txRole => {
        ...
        if (role.permissions.length) {
            await Promise.all(
                role.permissions.map(permission =>
                    role.$relatedQuery('permissions').unrelate().where({ id: permission.id })
                )
            );
        }
...
exports.upsertPermissions = function (id, permissionIds) {
    return Repository.tx(
        ...
        async (txRoleRepository, txPermissionRepository) => {
            const role = await internals.checkIfRoleExists(id);
