 ...
 */
internals.canRole = async function (roleName, action, resourceName) {
...
exports.canUser = async function (username, action, resourceName) {
    const user = await UserService.findOne({ withRoles: true }, { username });
    ...
    for (const role of user.roles) {
        if (await internals.canRole(role.name, action, resourceName)) {
            return true;
        }
    }
