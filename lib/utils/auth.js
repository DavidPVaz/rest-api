...
internals.token = async function (id, expiresIn) {
    console.log(
        `JWT authentication token for test user with id ${id}, ${
            expiresIn ? `expiring in ${expiresIn}` : 'with no expiration'
        } is:\n`
    );
    console.log(await exports.getToken(Number.parseInt(id), expiresIn));
};
...
const commands = {
    token: async () => internals.token(process.argv[3], process.argv[4]),
